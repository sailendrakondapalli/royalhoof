...
export default function OfferBanner() {
  ...
  return (
    <div className="sticky top-16 z-40 relative bg-gradient-to-r from-[#1A0A02] via-[#C5963A] to-[#1A0A02] text-[#1A0A02] overflow-hidden">
        ...
        <div className="absolute bottom-1 left-1/2 -translate-x-1/2 flex gap-1">
          {offers.map((_, i) => (
            <button
              ...
              className={`w-1 h-1 rounded-full transition-all ${i === current ? 'bg-[#FAF3E4] w-3' : 'bg-[#FAF3E4]/50'}`}
            />
          ))}
        </div>
      )}

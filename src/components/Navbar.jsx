...
export default function Navbar() {
  ...
  const navStyle = {
    background: scrolled
      ? "rgba(22, 8, 11, 0.97)"
      : (pathname === "/" ? "rgba(22, 8, 11, 0.82)" : "rgba(22, 8, 11, 0.97)"),
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 99999 }}>
      ...
      <div
        style={{
          ...
          height: "100%",
          width: "280px",
          background: "linear-gradient(180deg, #210B10 0%, #16080B 100%)",
      ...
      >
        ...
        <div style={{
          ...
          <button
            onClick={() => setMenuOpen(false)}
            style={{ color: "#F4EBDD", background: "none", border: "none", cursor: "pointer", padding: "4px" }}
          >
          ...
          {navLinks.map(item => (
            <Link
              ...
              style={{
                display: "block",
                padding: "13px 24px",
                color: isActive(item.to) ? "#E0C15A" : "#F4EBDD",
                textDecoration: "none",
                ...
                borderLeft: isActive(item.to) ? "2px solid #C9A227" : "2px solid transparent",
                transition: "background 0.2s, color 0.2s",
              }}
              onMouseEnter={e => {
                e.currentTarget.style.background = "rgba(201,162,39,0.08)"
                e.currentTarget.style.color = "#E0C15A"
              }}
              onMouseLeave={e => {
                e.currentTarget.style.background = "transparent"
                e.currentTarget.style.color = isActive(item.to) ? "#E0C15A" : "#F4EBDD"
              }}
            >
              {item.label}
            </Link>
        ...
        <div className="w-full px-6 lg:px-12 xl:px-20 h-20 flex items-center gap-6">
            ...
            <div className="hidden lg:flex items-center gap-1 flex-1 justify-center">
              {navLinks.map(item => (
                <Link key={item.to} to={item.to} onClick={closeAll}
                  ...
                  style={{
                    fontFamily: "'Cormorant Garamond', serif",
                    color: isActive(item.to) ? "#E0C15A" : "rgba(244,235,221,0.75)",
                  }}
                  onMouseEnter={e => { if (!isActive(item.to)) e.currentTarget.style.color = "#F4EBDD" }}
                  onMouseLeave={e => { if (!isActive(item.to)) e.currentTarget.style.color = "rgba(244,235,221,0.75)" }}
                >
                  {item.label}
                  {isActive(item.to) && (
                    <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-4 h-px bg-[#C9A227]" />
                  )}
                </Link>
              ))}
            </div>
          ...
          <div ref={searchRef} className="hidden lg:block relative w-72 ml-auto">
            <form onSubmit={handleSearch} className="relative">
              <Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-[#F4EBDD]/40 pointer-events-none" />
              <input
                type="text" value={searchQuery} onChange={handleSearchChange}
                placeholder="Search..."
                className="input-premium w-full rounded-sm pl-11 pr-4 py-2.5 text-sm text-[#F4EBDD] placeholder-[#F4EBDD]/30"
              />
            </form>
            {suggestions.length > 0 && (
              <div 
                ...
                style={{ 
                  background: "linear-gradient(145deg, #210B10 0%, #16080B 100%)", 
                  border: "1px solid rgba(201, 162, 39, 0.25)",
                  boxShadow: "0 12px 32px rgba(0, 0, 0, 0.5)" 
                }}>
                {suggestions.map(p => (
                  <button key={p.id} onClick={() => handleSuggestionClick(p)}
                    className="w-full flex items-center gap-3 px-4 py-3 hover:bg-[rgba(201,162,39,0.06)] transition-colors text-left">
                    {p.images?.[0] && (
                      <img src={p.images[0]} alt="" className="w-10 h-10 object-cover rounded-sm flex-shrink-0" 
                        onError={e => { e.target.style.display = "none" }} />
                    )}
                    <div className="flex-1 min-w-0">
                      <p className="text-[#F4EBDD] text-sm font-medium truncate" style={{ fontFamily: "'Inter', sans-serif" }}>{p.name}</p>
                      <p className="text-[#B9AFA3] text-xs">{p.category}</p>
                    </div>
                    <span className="text-[#F4EBDD] text-sm font-semibold flex-shrink-0">
          ...
          <div className="flex items-center gap-1 flex-shrink-0 ml-auto lg:ml-0">
            ...
            <button 
              ...
              style={{ color: "#F4EBDD", background: "none", border: "none", cursor: "pointer" }}

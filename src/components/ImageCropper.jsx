...
export default function ImageCropper({ imageSrc, aspect = 16 / 5, onCrop, onCancel, title = "Crop Image" }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/85 z-[200] flex items-center justify-center p-4">
      <div className="bg-[#FAF3E4] border border-[#765334] rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-3 border-b border-[#765334]">
        ...
        <div className="relative w-full" style={{ height: 340, background: "#1A0A02" }}>
          <Cropper
            ...
            style={{
              ...
              cropAreaStyle: { border: "2px solid #C5963A" },
            }}
          />
        </div>

        {/* Zoom slider */}
        <div className="flex items-center gap-3 px-5 py-3 border-t border-[#765334]">
          <ZoomOut size={15} className="text-white/60 flex-shrink-0" />
          <input
            type="range" min={1} max={3} step={0.05}
            value={zoom}
            onChange={e => setZoom(Number(e.target.value))}
            className="flex-1 accent-[#C5963A]"
          />
        ...
        <div className="flex gap-2 px-5 pb-4">
          <button onClick={onCancel}
            className="flex-1 py-2 border border-[#765334] text-white/70 rounded-lg text-sm hover:bg-[#3D1F0A] transition-all">
            Cancel
          </button>
          <button onClick={handleCrop} disabled={cropping}
            ...
            style={{ background: "#C5963A", color: "#1A0A02" }}>
            {cropping

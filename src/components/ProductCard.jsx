...
function TagBadges({ tags }) {
  ...
  return (
    <div className="flex flex-wrap gap-1">
      ...
      {tags.includes('nepal') && (
        <span className="flex items-center gap-1 text-[0.625rem] font-semibold px-2 py-0.5 rounded-sm bg-[#C9A227]/15 text-[#C9A227] border border-[#C9A227]/30"
          style={{ fontFamily: "'Inter', sans-serif" }}>
  ...
  return (
    ...
    >
      <Link to={`/products/${product.id}`} className="flex flex-col flex-1">
        {/* Image Container */}
        <div className="relative overflow-hidden aspect-square bg-[#16080B] flex-shrink-0 image-zoom-wrapper">
          {mediaIsVideo ? (
          ...
          {/* Overlay on Hover */}
          <div className="absolute inset-0 bg-gradient-to-t from-[#16080B]/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />

          {/* Out of Stock Overlay */}
          {product.stock === 0 && (
            <div className="absolute inset-0 bg-[#16080B]/80 flex items-center justify-center backdrop-blur-sm">
              <span className="text-[#F4EBDD] text-xs font-semibold bg-[#16080B] px-4 py-2 rounded-sm border border-[#B9AFA3]/20"
                style={{ fontFamily: "'Inter', sans-serif" }}>
                Out of Stock
              </span>
            </div>
          ...
          {!product.tags?.includes('certified') && product.tags?.includes('new') && (
            <span className="absolute top-3 left-3 bg-[#C9A227] text-[#F4EBDD] text-[0.625rem] px-2 py-1 rounded-sm font-bold backdrop-blur-sm"
              style={{ fontFamily: "'Inter', sans-serif" }}>
              New
            </span>
          )}
          ...
          <button onClick={onWishlist}
            className={`absolute top-3 right-3 p-2 rounded-sm transition-all duration-300 backdrop-blur-sm ${
              wishlisted 
                ...
                : 'bg-[#16080B]/80 text-[#F4EBDD] hover:text-red-400 hover:bg-[#16080B]'
            }`}>
        ...
        <div className="p-4 flex flex-col flex-1">
          {/* Category */}
          <p className="text-[0.625rem] text-[#C9A227] mb-2 uppercase tracking-[0.15em] font-semibold"
            style={{ fontFamily: "'Inter', sans-serif" }}>
            {product.category}
          </p>
          
          {/* Title */}
          <h3 className="text-[#F4EBDD] text-sm font-medium line-clamp-2 mb-3 group-hover:text-[#C9A227] transition-colors leading-snug flex-1"
            style={{ fontFamily: "'Inter', sans-serif" }}>
            {product.name}
          </h3>
          ...
          <div className="mt-auto">
            <div className="flex items-center gap-2">
              <span className="text-[#F4EBDD] font-semibold text-base" style={{ fontFamily: "'Inter', sans-serif" }}>
                {formatINR(product.price)}
              </span>
              {origPrice && origPrice > product.price && (
                <span className="text-[#B9AFA3]/60 text-xs line-through">
                  {formatINR(origPrice)}
                </span>
              )}
            </div>
            <p className="text-[0.625rem] mt-1" 
              style={{ 
                color: product.delivery_charge ? "#B9AFA3" : "#4ADE80",
        ...
        <button onClick={onAddToCart} disabled={product.stock === 0}
          className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-sm text-xs font-semibold transition-all duration-300 disabled:opacity-40 disabled:cursor-not-allowed uppercase tracking-wider ${
            inCart
              ...
              : 'bg-[#C9A227] hover:bg-[#F4EBDD] text-[#16080B]'
          }`}
          style={{ fontFamily: "'Inter', sans-serif" }}>
          {inCart ? (
    ...
    <motion.div
      ...
      <Link to={`/products/${product.id}`}>
        <div className="flex items-stretch gap-4 p-4">
          {/* Image */}
          <div className="relative flex-shrink-0 w-28 h-28 rounded-sm overflow-hidden bg-[#16080B] image-zoom-wrapper">
            {mediaIsVideo ? (
            ...
            {product.stock === 0 && (
              <div className="absolute inset-0 bg-[#16080B]/80 flex items-center justify-center backdrop-blur-sm">
                <span className="text-[0.625rem] font-bold text-[#F4EBDD] bg-[#16080B] px-2 py-1 rounded-sm border border-[#B9AFA3]/20"
                  style={{ fontFamily: "'Inter', sans-serif" }}>
                  OOS
                </span>
              </div>
          ...
          <div className="flex-1 min-w-0 flex flex-col justify-between py-1">
            <div className="flex items-center gap-2 mb-1">
              {product.custom_id && (
                <span className="font-mono text-[0.625rem] text-[#B9AFA3] bg-[#16080B] px-2 py-0.5 rounded-sm border border-[#B9AFA3]/15"
                  style={{ fontFamily: "'Courier New', monospace" }}>
                  {product.custom_id}
                </span>
              )}
              <span className="text-[0.625rem] text-[#C9A227] font-semibold uppercase tracking-wider px-2 py-0.5 bg-[#C9A227]/10 rounded-sm border border-[#C9A227]/20"
                style={{ fontFamily: "'Inter', sans-serif" }}>
                {product.category}
              </span>
            </div>
            
            <h3 className="text-[#F4EBDD] text-sm font-medium line-clamp-2 group-hover:text-[#C9A227] transition-colors leading-tight mb-2"
              style={{ fontFamily: "'Inter', sans-serif" }}>
              {product.name}
            </h3>
            
            {product.size && (
              <p className="text-[#B9AFA3] text-xs mb-2" style={{ fontFamily: "'Inter', sans-serif" }}>
                Size: <span className="font-medium text-[#F4EBDD]">{product.size} mm</span>
              </p>
            )}
            ...
            <button onClick={onWishlist}
              className={`p-2 rounded-sm transition-all duration-300 ${
                wishlisted 
                  ...
                  : 'text-[#F4EBDD]/50 hover:text-red-400 hover:bg-red-900/20'
              }`}>

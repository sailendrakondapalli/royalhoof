      ...
      <div className="flex items-center justify-between px-5 py-4 cursor-pointer hover:bg-[#F4E9D2] transition-colors"
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[#765334] text-xs">{formatDate(order.created_at)}</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden">
            <div className="border-t border-[rgba(8,43,73,0.15)] px-5 py-4 space-y-4">
              ...
              <div className="space-y-2">
                <p className="text-[#765334] text-xs font-semibold uppercase tracking-wider">Items</p>
                {(order.order_items || []).map(item => (
                  <div key={item.id} className="flex items-center gap-3 bg-[#F4E9D2] rounded-xl p-2.5">
              ...
              {/* Address */}
              {addr.full_name && (
                <div className="bg-[#F4E9D2] rounded-xl p-3">
                  <p className="text-[#765334] text-xs font-semibold uppercase tracking-wider mb-1.5">Delivery Address</p>
                  <p className="text-[#292725] text-sm font-medium">{addr.full_name}</p>
                  <p className="text-[#765334] text-xs">{addr.phone}</p>
  ...
  return (
    <>
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold text-[#292725] mb-6" style={{ fontFamily: 'Georgia, serif' }}>My Orders</h1>
          ...
          <div className="text-center py-20">
            ...
            {search ? (
              <>
                ...
                <button onClick={() => setSearch('')} className="mt-3 text-[#C5963A] text-xs hover:underline">Clear search</button>
              </>
            ) : (
              <>

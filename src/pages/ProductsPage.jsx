    ...
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="flex items-start justify-between mb-3 gap-3">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-[#C5963A] leading-tight" style={{ fontFamily: 'Georgia, serif' }}>
              {headingText}
            </h1>
            {search && (
          ...
          <div className="flex items-center gap-2 flex-shrink-0 mt-1">
            {/* Result count */}
            <span className="text-[#765334] text-xs hidden sm:block">
              {loading ? '?' : `${products.length} item${products.length !== 1 ? 's' : ''}`}
            </span>

            {/* View toggle */}
            <div className="flex items-center bg-[#FAF3E4] rounded-lg p-0.5 border border-[#765334]">
              <button
                onClick={() => setViewMode('grid')}
                title="Grid view"
                className={`p-1.5 rounded-md transition-all ${viewMode === 'grid' ? 'bg-[#C5963A] text-[#1A0A02] shadow-sm' : 'text-[#DDB87A] hover:text-[#C5963A]'}`}
              >
                <LayoutGrid size={15} />
              </button>
              <button
                ...
                title="List view"
                className={`p-1.5 rounded-md transition-all ${viewMode === 'list' ? 'bg-[#C5963A] text-[#1A0A02] shadow-sm' : 'text-[#DDB87A] hover:text-[#C5963A]'}`}
              >
                <List size={15} />
              </button>
            </div>
          ...
          <div className="flex items-center gap-1.5 flex-nowrap min-w-0 flex-1">
            <button
              ...
              className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold transition-all border ${
                !category
                  ? 'bg-[#C5963A] text-[#1A0A02] border-[#C5963A] shadow-sm'
                  : 'bg-[#FAF3E4] text-[#DDB87A] hover:text-[#C5963A] border-[#765334] hover:border-[#C5963A]/60'
              }`}
            >
              All
            </button>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <div className="relative" ref={sortRef}>
              <button
                ...
                className="flex items-center gap-1.5 bg-[#FAF3E4] border border-[#765334] text-[#DDB87A] text-xs rounded-lg px-3 py-2 hover:border-[#C5963A]/60 transition-colors whitespace-nowrap"
              >
              ...
              <AnimatePresence>
                {sortOpen && (
                  <motion.div
                    ...
                    className="absolute right-0 top-full mt-1.5 w-44 bg-[#FAF3E4] border border-[#765334] rounded-xl shadow-xl z-30 py-1.5 overflow-hidden"
                  >
                    {SORT_OPTIONS.map(o => (
                      <button
                        key={o.value}
                        ...
                        className={`w-full text-left px-4 py-2 text-xs transition-colors ${
                          sort === o.value
                            ? 'bg-[#3D1F0A] text-[#C5963A] font-semibold'
                            : 'text-[#DDB87A] hover:bg-[#3D1F0A] hover:text-[#C5963A]'
                        }`}
                      >
        ...
        <div className="flex gap-6">
          ...
          <aside className="hidden lg:block w-[220px] flex-shrink-0">
            <div className="sticky top-20 bg-[#FAF3E4] rounded-2xl border border-[#765334] overflow-hidden shadow-sm">
              <div className="px-4 py-3 bg-[#C5963A]">
              ...
              <nav className="py-2">
                <button
                  ...
                  className={`w-full flex items-center justify-between px-4 py-2.5 text-sm transition-all text-left ${
                    !category
                      ? 'text-[#C5963A] font-semibold bg-[#3D1F0A] border-l-2 border-[#C5963A]'
                      : 'text-[#DDB87A] hover:text-[#C5963A] hover:bg-[#3D1F0A]'
                  }`}
                >
                  <span>All</span>
                  <span className="text-[10px] text-[#DDB87A] bg-[#1A0A02] px-1.5 py-0.5 rounded-full border border-[#765334]">
                    ...
                    >
                      ...
                      {count > 0 && (
                        <span className={`flex-shrink-0 text-[10px] px-1.5 py-0.5 rounded-full ${
                          active ? 'bg-[#C5963A] text-[#1A0A02]' : 'text-[#DDB87A] bg-[#1A0A02] border border-[#765334]'
                        }`}>
                          {count}
                        </span>
                      )}
          ...
          <div className="flex-1 min-w-0">
            ...
            {!loading && products.length > 0 && (
              <div className="flex items-center justify-between mb-4">
                <p className="text-[#DDB87A] text-xs">
                  Showing <span className="text-[#C5963A] font-medium">{(page - 1) * pageSize + 1}?{Math.min(page * pageSize, products.length)}</span> of <span className="text-[#C5963A] font-medium">{products.length}</span>
                </p>
                <div className="flex items-center gap-2">
                  ...
                  <select
                    ...
                    className="bg-[#FAF3E4] border border-[#765334] text-[#DDB87A] text-xs rounded-lg px-2 py-1.5 focus:outline-none focus:border-[#C5963A]"
                  >
            ...
            {loading ? (
              ...
              >
                ...
                <h3 className="text-[#C5963A] text-lg font-bold mb-1" style={{ fontFamily: 'Georgia, serif' }}>
                ...
                <button
                  onClick={clearFilters}
                  className="px-6 py-2.5 bg-[#C5963A] text-[#1A0A02] rounded-xl text-sm font-semibold hover:bg-[#E5A020] transition-all shadow-md hover:shadow-[0_4px_16px_rgba(200,134,10,0.4)]"
                >
                ...
                {totalPages > 1 && (
                  <div className="flex items-center justify-center gap-2 mt-10 flex-wrap">
                    <button
                      ...
                      disabled={page === 1}
                      className="flex items-center gap-1 px-3 py-2 text-xs rounded-lg border border-[#765334] text-[#DDB87A] hover:border-[#C5963A] disabled:opacity-40 transition-all"
                    >
                    ...
                    {Array.from({ length: totalPages }, (_, i) => i + 1)
                      ...
                      .map((p, idx) =>
                        p === '?'
                          ? <span key={`ellipsis-${idx}`} className="text-[#765334] text-xs px-1">?</span>
                          : (
                            <button
                              key={p}
                              ...
                              className={`w-8 h-8 text-xs rounded-lg border transition-all ${
                                p === page
                                  ? 'bg-[#C5963A] text-[#1A0A02] border-[#C5963A] shadow-sm'
                                  : 'border-[#765334] text-[#DDB87A] hover:border-[#C5963A]'
                              }`}
                            >
                              {p}
                            </button>

...
const COLORS = ['#C5963A', '#B9AFA3', '#9A8870', '#7A6A54', '#5A4A38', '#C4B09A', '#E8D8C4', '#F0E8D8']

const TooltipStyle = {
  contentStyle: { background: '#F4E9D2', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, fontSize: 11, color: '#292725' },
...
export default function AdminAnalytics() {
  ...
  if (!stats) {
    return (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: 240 }}>
        <div style={{ width: 32, height: 32, border: "2px solid #C5963A", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.7s linear infinite" }} />
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "1.75rem", fontWeight: 700, color: "#292725" }}>Analytics</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={i} style={{ background: "#F4E9D2", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 8, padding: 20 }}>
            <k.icon size={18} style={{ color: "#C5963A", marginBottom: 12 }} />
            <p style={{ fontSize: "1.5rem", fontWeight: 700, color: "#292725", fontFamily: "'Inter', sans-serif" }}>{k.value}</p>
      ...
      {/* Revenue Area Chart */}
      <div style={{ background: "#F4E9D2", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 8, padding: 20 }}>
        <h3 style={{ color: "#292725", fontWeight: 600, marginBottom: 16, fontSize: "0.9375rem", fontFamily: "'Inter', sans-serif" }}>Revenue Trend (Last 14 Days)</h3>
        <ResponsiveContainer width="100%" height={260}>
          <AreaChart data={stats.last14Days}>
            <defs>
              <linearGradient id="accentGrad" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor="#C5963A" stopOpacity={0.25} />
                <stop offset="95%" stopColor="#C5963A" stopOpacity={0} />
              </linearGradient>
            </defs>
            ...
            <Area type="monotone" dataKey="revenue" stroke="#C5963A" fill="url(#accentGrad)" strokeWidth={2} name="Revenue" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div style={{ background: "#F4E9D2", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 8, padding: 20 }}>
          <h3 style={{ color: "#292725", fontWeight: 600, marginBottom: 16, fontSize: "0.9375rem", fontFamily: "'Inter', sans-serif" }}>Weekly Orders</h3>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={weeklyData}>
              ...
              <Bar dataKey="orders" fill="#C5963A" radius={[4, 4, 0, 0]} name="Orders" />
            </BarChart>
          </ResponsiveContainer>
        </div>

        <div style={{ background: "#F4E9D2", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 8, padding: 20 }}>
          <h3 style={{ color: "#292725", fontWeight: 600, marginBottom: 16, fontSize: "0.9375rem", fontFamily: "'Inter', sans-serif" }}>Revenue by Category</h3>
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
      ...
      {/* Location Analytics */}
      <div style={{ background: "#F4E9D2", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 8, padding: 20 }}>
        <h3 style={{ color: "#292725", fontWeight: 600, marginBottom: 16, fontSize: "0.9375rem", display: "flex", alignItems: "center", gap: 8, fontFamily: "'Inter', sans-serif" }}>
          <MapPin size={15} style={{ color: "#C5963A" }} /> Location Analytics — Top Cities
        </h3>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={stats.cityData} layout="vertical">
              ...
              <Tooltip {...TooltipStyle} />
              <Bar dataKey="count" fill="#B9AFA3" radius={[0, 4, 4, 0]} name="Orders" />
            </BarChart>
          </ResponsiveContainer>
          ...
          <div className="space-y-3">
            {stats.cityData.map((c, i) => (
              <div key={c.city} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <span style={{ color: "#C5963A", fontSize: "0.75rem", width: 20, fontWeight: 700 }}>{i + 1}</span>
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                    <span style={{ color: "#292725", fontSize: "0.75rem" }}>{c.city}</span>
      ...
      {/* Top Products */}
      <div style={{ background: "#F4E9D2", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 8, padding: 20 }}>
        <h3 style={{ color: "#292725", fontWeight: 600, marginBottom: 16, fontSize: "0.9375rem", fontFamily: "'Inter', sans-serif" }}>Best Selling Products</h3>
        <ResponsiveContainer width="100%" height={240}>

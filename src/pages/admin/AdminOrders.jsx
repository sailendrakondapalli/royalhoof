      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }}
            className="absolute top-full mt-1 right-0 z-20 bg-[#FAF3E4] border border-gray-200 rounded-xl overflow-hidden shadow-xl min-w-[130px]">
  ...
  return (
    <div className="bg-orange-50 border-2 border-orange-400 rounded-xl p-4 space-y-3">
      ...
      <input value={trackingId} onChange={e => setTrackingId(e.target.value)} placeholder="Tracking ID (required)"
        className="w-full bg-[#FAF3E4] border-2 border-orange-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-orange-500" />
      <label className="flex items-center gap-2 p-2.5 border-2 border-dashed border-orange-300 hover:border-orange-500 rounded-lg cursor-pointer bg-[#FAF3E4]">
...
function OrderRow({ order, expanded, onToggle, onStatusUpdate, onVerify, onReject, onNotify, onScreenshot, onTrackingSave }) {
  ...
  return (
    <div className={`border rounded-xl overflow-hidden mb-2 ${needsVerification ? "border-orange-400/50 bg-orange-50/30" : isCancelled ? "border-red-200" : "border-gray-200 bg-[#FAF3E4]"}`}>
      {/* Row header */}
          ...
          <motion.div initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }} className="overflow-hidden">
            <div className="border-t border-gray-200 p-4 space-y-3">
                ...
                <div className="bg-orange-500/10 border border-orange-500/20 rounded-lg p-3 space-y-3">
                  ...
                  {order.payment_screenshot_url && (
                    <div className="space-y-2">
                      <img src={order.payment_screenshot_url} alt="Payment" className="w-full max-h-48 object-contain rounded-lg border border-orange-300 bg-[#FAF3E4] cursor-pointer" onClick={() => onScreenshot(order.payment_screenshot_url)} />
                  ...
                  {order.upi_ref && <p className="text-gray-500 text-xs">UPI Ref: <span className="font-mono text-[#292725]">{order.upi_ref}</span></p>}
              ...
              {order.payment_status === "paid" && (
                <div className="bg-[#FAF3E4] border border-gray-200 rounded-xl p-4 space-y-3">
                  <p className="text-[#292725] text-xs font-semibold">Order Status</p>
              ...
              {addr.full_name && (
                <div className="bg-gray-50 rounded-lg p-2 text-xs text-gray-400">
                  <p className="text-[#292725]">{addr.full_name} &middot; {addr.phone}</p>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "1.75rem", fontWeight: 700, color: "#292725" }}>
      ...
      <div className="relative">
        ...
        <input value={search} onChange={e => { setSearch(e.target.value); setPage(1) }}
          ...
          style={{ width: "100%", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 5, paddingLeft: 36, paddingRight: 16, paddingTop: 10, paddingBottom: 10, fontSize: "0.875rem", color: "#292725", outline: "none", fontFamily: "'Inter', sans-serif", boxSizing: "border-box" }} />
      </div>
      ...
      {/* Orders list */}
      <div style={{ background: "#F4E9D2", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 8, padding: 16 }}>
        {filtered.length === 0 ? (

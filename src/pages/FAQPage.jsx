        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="text-center mb-12">
            ...
            <h1 className="heading-editorial text-4xl mb-4">
              <span style={{ color: "#F4EBDD" }}>Frequently Asked</span> <span style={{ color: "#D8C7A0", fontStyle: "italic" }}>Questions</span>
            </h1>
          ...
          <div className="mb-10">
            <div className="relative">
              <Search size={20} className="absolute left-4 top-1/2 -translate-y-1/2 text-[#B9AFA3]" />
              <input
                type="text"
                value={searchTerm}
                onChange={e => setSearchTerm(e.target.value)}
                placeholder="Search for questions..."
                className="input-premium w-full rounded-sm pl-12 pr-4 py-4 text-[#F4EBDD] placeholder-[#B9AFA3]/50"
              />
            </div>
          </div>
          ...
          {filteredFAQs.length === 0 ? (
            <div className="text-center py-16 equestrian-card rounded-lg">
              <p className="text-[#B9AFA3]">No results found for "{searchTerm}"</p>
              <button
                onClick={() => setSearchTerm('')}
                className="mt-4 text-[#C9A227] hover:text-[#8A6640] text-sm font-medium"
              >
                Clear search
              </button>
            </div>
            ...
            <div className="space-y-8">
              {filteredFAQs.map((category, catIndex) => (
                <div key={catIndex}>
                  {/* Category Title */}
                  <h2 className="text-[#C9A227] font-medium text-lg mb-4 uppercase tracking-wider">
                    {category.category}
                  </h2>
                      ...
                      return (
                        <div key={qIndex} className="equestrian-card rounded-lg overflow-hidden">
                          <button
                            ...
                            className="w-full px-6 py-4 flex items-center justify-between text-left hover:bg-[#2A0D13] transition-colors"
                          >
                            <span className="text-[#F4EBDD] font-medium pr-4">{item.q}</span>
                            <ChevronDown
                              size={20}
                              className={`text-[#C9A227] flex-shrink-0 transition-transform ${
                                isOpen ? 'rotate-180' : ''
                              }`}
                            />
                          </button>
                          {isOpen && (
                            <div className="px-6 pb-4 pt-2 border-t border-[#2A0D13]">
                              <p className="text-[#C9A227] leading-relaxed">{item.a}</p>
                            </div>
                          )}
                        </div>
                      )
          ...
          <div className="mt-16 text-center equestrian-card rounded-lg p-10">
            <h2 className="heading-editorial text-2xl mb-4">
              <span style={{ color: "#F4EBDD" }}>Still Have</span> <span style={{ color: "#D8C7A0", fontStyle: "italic" }}>Questions?</span>
            </h2>
            ...
            <div className="flex flex-wrap gap-4 justify-center">
              <a
                href="/enquiry"
                className="btn-primary-equestrian inline-block"
              >
                Send Enquiry
              </a>
              <a
                href="tel:+919043700776"
                className="btn-secondary-equestrian inline-block"
              >
                Call Us
              </a>
            </div>

        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="text-center mb-12">
            ...
            <h1 className="heading-editorial text-4xl mb-4">
              <span style={{ color: "#292725" }}>Frequently Asked</span> <span style={{ color: "#D8C7A0", fontStyle: "italic" }}>Questions</span>
            </h1>
          ...
          <div className="mb-10">
            <div className="relative">
              <Search size={20} className="absolute left-4 top-1/2 -translate-y-1/2 text-[#765334]" />
              <input
                type="text"
                value={searchTerm}
                onChange={e => setSearchTerm(e.target.value)}
                placeholder="Search for questions..."
                className="input-premium w-full rounded-sm pl-12 pr-4 py-4 text-[#292725] placeholder-[#B9AFA3]/50"
              />
            </div>
          </div>
          ...
          {filteredFAQs.length === 0 ? (
            <div className="text-center py-16 equestrian-card rounded-lg">
              <p className="text-[#765334]">No results found for "{searchTerm}"</p>
              <button
                onClick={() => setSearchTerm('')}
                className="mt-4 text-[#C5963A] hover:text-[#8A6640] text-sm font-medium"
              >
                Clear search
              </button>
            </div>
            ...
            <div className="space-y-8">
              {filteredFAQs.map((category, catIndex) => (
                <div key={catIndex}>
                  {/* Category Title */}
                  <h2 className="text-[#C5963A] font-medium text-lg mb-4 uppercase tracking-wider">
                    {category.category}
                  </h2>
                      ...
                      return (
                        <div key={qIndex} className="equestrian-card rounded-lg overflow-hidden">
                          <button
                            ...
                            className="w-full px-6 py-4 flex items-center justify-between text-left hover:bg-[#FAF3E4] transition-colors"
                          >
                            <span className="text-[#292725] font-medium pr-4">{item.q}</span>
                            <ChevronDown
                              size={20}
                              className={`text-[#C5963A] flex-shrink-0 transition-transform ${
                                isOpen ? 'rotate-180' : ''
                              }`}
                            />
                          </button>
          ...
          <div className="mt-16 text-center equestrian-card rounded-lg p-10">
            <h2 className="heading-editorial text-2xl mb-4">
              <span style={{ color: "#292725" }}>Still Have</span> <span style={{ color: "#D8C7A0", fontStyle: "italic" }}>Questions?</span>
            </h2>

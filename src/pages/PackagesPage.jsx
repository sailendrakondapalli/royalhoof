  ...
  const loadPackages = async () => {
    ...
    try {
      ...
      const adult = (data || []).filter(pkg => pkg.package_type === 'adult').map(pkg => ({
        ...
        color: pkg.is_popular ? '#C9A227' : '#B9AFA3',
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-12">
            ...
            <h1 className="heading-editorial text-4xl mb-4">
              <span style={{ color: "#F4EBDD" }}>Packages &</span> <span style={{ color: "#D8C7A0", fontStyle: "italic" }}>Memberships</span>
            </h1>
            ...
            <>
              {loading ? (
                ...
                <div className="text-center py-20 equestrian-card rounded-lg mb-16">
                  <p className="text-[#B9AFA3] text-lg">No adult packages available</p>
                  ...
                  {adultPackages.map(pkg => (
                    <div 
                      ...
                      className={`equestrian-card rounded-lg p-8 relative ${
                        pkg.popular ? 'ring-2 ring-[#C9A227]' : ''
                      }`}
                    >
                      {pkg.popular && (
                        <div className="absolute -top-4 left-1/2 -translate-x-1/2">
                          <span className="bg-[#C9A227] text-white px-4 py-1 rounded-full text-xs font-medium uppercase flex items-center gap-1">
                      ...
                      <ul className="space-y-3 mb-8">
                        {pkg.features.map((feature, i) => (
                          <li key={i} className="flex items-start gap-3 text-[#C9A227] text-sm">
                            <Check size={18} className="text-[#C9A227] flex-shrink-0 mt-0.5" />
                            <span>{feature}</span>
                          </li>
                        ))}
                      </ul>
                      ...
                      <button
                        ...
                        className={`w-full py-3 rounded-lg font-medium uppercase tracking-wider transition-colors ${
                          pkg.popular
                            ? 'bg-[#C9A227] hover:bg-[#8A6640] text-white'
                            : 'bg-[#2A0D13] hover:bg-[#2A0D13] text-[#F4EBDD]'
                        }`}
                      >
                        Get Started
                      </button>
          ...
          {activeTab === 'kids' && (
            <>
              {loading ? (
                ...
                <div className="text-center py-20 equestrian-card rounded-lg mb-16">
                  <p className="text-[#B9AFA3] text-lg">No kids packages available</p>
                  <p className="text-[#B9AFA3] text-sm mt-2">Check back soon for updates!</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl mx-auto mb-16">
                  {kidsPackages.map(pkg => (
                    <div key={pkg.id} className="equestrian-card rounded-lg p-8">
                      ...
                      <p className="text-[#C9A227] text-sm mb-4 font-medium">{pkg.ageGroup}</p>
                      ...
                      <button
                        onClick={() => handleEnquiry(pkg)}
                        className="w-full bg-[#C9A227] hover:bg-[#8A6640] text-white py-3 rounded-lg font-medium uppercase tracking-wider transition-colors"
                      >
                        Get Started
                      </button>
                    </div>
          ...
          <div className="equestrian-card rounded-lg p-10 text-center">
            <h2 className="heading-editorial text-2xl mb-6">
              <span style={{ color: "#F4EBDD" }}>Why Choose Our</span> <span style={{ color: "#D8C7A0", fontStyle: "italic" }}>Memberships?</span>
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
              {[
                ...
                <div key={i}>
                  <h4 className="text-[#C9A227] font-medium mb-2">{benefit.title}</h4>
                  <p className="text-[#C9A227] text-sm">{benefit.desc}</p>
                </div>
              ))}
            </div>
            <a

...
export default function ProductDetailPage() {
  ...
  const tags = [
    { label: product.category, bg: 'bg-[#C5963A]/10', border: 'border-[#C5963A]/30', text: 'text-[#C5963A]' },
    ...(product.tags || []).slice(0, 3).map((t, i) => {
      const palettes = [
        { bg: 'bg-[#C5963A]/10', border: 'border-[#C5963A]/30', text: 'text-[#C5963A]' },
    ...
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-2 text-xs text-[#DDB87A]/60 mb-6">
          ...
          <span>/</span>
          <Link to={`/products?category=${encodeURIComponent(product.category)}`} className="hover:text-[#C5963A] transition-colors">{product.category}</Link>
          <span>/</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div className="space-y-3">
            {/* Main image */}
            <div className="relative aspect-square rounded-2xl overflow-hidden bg-[#1A0A02] border border-[#765334]">
              ...
              {images.length > 1 && (
                <>
                  <button onClick={() => setImgIdx(i => (i - 1 + images.length) % images.length)}
                    className="absolute left-3 top-1/2 -translate-y-1/2 w-8 h-8 bg-[#FAF3E4]/80 rounded-full flex items-center justify-center shadow hover:bg-[#FAF3E4] transition-all">
                    <ArrowLeft size={14} />
                  </button>
                  <button onClick={() => setImgIdx(i => (i + 1) % images.length)}
                    className="absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 bg-[#FAF3E4]/80 rounded-full flex items-center justify-center shadow hover:bg-[#FAF3E4] transition-all">
                    <ArrowRight size={14} />
                  </button>
                </>
              )}
              ...
              <div className="flex gap-2 flex-wrap">
                {images.map((img, i) => (
                  <button key={i} onClick={() => setImgIdx(i)}
                    className={`w-16 h-16 rounded-lg overflow-hidden border-2 transition-all ${i === imgIdx ? 'border-[#C5963A]' : 'border-transparent opacity-60 hover:opacity-100'}`}>
                    {isVideoUrl(img) ? (
          ...
          {/* Right - info */}
          <div>
            <p className="text-[#C5963A] text-xs uppercase tracking-widest mb-2">{product.category}</p>
            <h1 className="text-3xl font-bold text-[#C5963A] mb-3" style={{ fontFamily: 'Georgia, serif' }}>{product.name}</h1>
            {product.custom_id && <p className="text-[#DDB87A]/60 text-xs font-mono mb-2">ID: {product.custom_id}</p>}

            {/* Rating */}
            <div className="flex items-center gap-2 mb-4">
              <div className="flex text-[#C5963A]">{Array(5).fill(0).map((_, i) => <Star key={i} size={14} fill="currentColor" />)}</div>
              <span className="text-[#DDB87A]/60 text-xs">(Authentic &amp; Certified)</span>
            </div>

            <div className="mb-4">
              <div className="flex items-center gap-3">
                <p className="text-3xl font-bold text-[#C5963A]">{formatINR(product.price)}</p>
              ...
              <p className="text-sm mt-1" style={{ color: product.delivery_charge ? "#765334" : "#16a34a" }}>
            ...
            <div className="grid gap-3 mb-6 text-sm">
              {product.size && (
                <div className="bg-[#FAF3E4] rounded-lg p-3 border border-[#765334]">
                  <p className="text-[#DDB87A]/60 text-xs mb-2">Available Variants</p>
                  <div className="flex flex-wrap gap-2">
                    {product.size.split(',').map(s => s.trim()).filter(Boolean).map(s => (
                      <span key={s} className="px-3 py-1 bg-[#C5963A]/10 border border-[#C5963A]/30 text-[#C5963A] text-xs rounded-full font-medium">{s}</span>
                    ))}
                  </div>
                </div>
              )}
            ...
            <div className="flex gap-3 mb-4">
              <button onClick={handleAddToCart} disabled={product.stock === 0 || addingCart}
                className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-semibold text-sm transition-all disabled:opacity-40 disabled:cursor-not-allowed ${
                  inCart ? 'bg-green-700 hover:bg-green-600 text-white' : 'bg-[#C5963A] hover:bg-[#E5A020] text-[#1A0A02]'
                }`}>
                {addingCart
              ...
              <button onClick={handleWishlist}
                className={`w-12 h-12 rounded-xl border-2 flex items-center justify-center transition-all flex-shrink-0 ${
                  wishlisted ? 'bg-red-500 border-red-500 text-white' : 'border-[#765334] text-[#DDB87A] hover:border-red-400 hover:text-red-400'
                }`}>
              ...
              <button onClick={() => {
                ...
                className="w-12 h-12 rounded-xl border-2 border-[#765334] text-[#DDB87A] flex items-center justify-center hover:border-[#C5963A] hover:text-[#C5963A] transition-all flex-shrink-0">
                <Share2 size={18} />
              </button>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 text-center">
              {[
                ...
                <div key={b.label} className="bg-[#FAF3E4] border border-[#765334] rounded-xl py-2.5 px-1">
        ...
        {related.length > 0 && (
          <section className="mt-14">
            <h2 className="text-2xl font-bold text-[#C5963A] mb-6" style={{ fontFamily: 'Georgia, serif' }}>
              More {product.category}
            </h2>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {related.map(p => (
                <Link key={p.id} to={`/products/${p.id}`}
                  className="group bg-[#FAF3E4] border border-[#765334] rounded-2xl overflow-hidden hover:border-[#C5963A]/60 hover:shadow-[0_4px_20px_rgba(200,134,10,0.15)] transition-all">
                  ...
                  <div className="p-3">
                    <p className="text-white text-sm font-semibold line-clamp-2 group-hover:text-[#C5963A] transition-colors">{p.name}</p>
                    <p className="text-[#C5963A] font-bold mt-1">{formatINR(p.price)}</p>
                  </div>
                </Link>
              ))}
            </div>

  ...
  return (
    <form onSubmit={handleSubmit} className="border border-[rgba(8,43,73,0.15)] rounded-xl p-4 bg-[#F4E9D2] space-y-3">
      ...
      <label className="flex items-center gap-2 text-xs text-gray-400 cursor-pointer">
        <input type="checkbox" checked={form.is_default} onChange={e => setForm(f => ({ ...f, is_default: e.target.checked }))} className="accent-[#C5963A]" />
...
export default function CheckoutPage() {
  ...
  if (step === "success") {
    return (
      <div className="max-w-lg mx-auto px-4 py-20 text-center">
        ...
        <h2 className="text-3xl font-bold text-[#292725] mb-3" style={{ fontFamily: "Georgia, serif" }}>Order Placed!</h2>
            ...
            return (
              <div className="space-y-4">
                ...
                <div className="rounded-2xl overflow-hidden shadow-sm">
                  ...
                  <div className="bg-[#FAF3E4] px-6 py-6 space-y-5">
                    {/* Amount */}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[#765334] text-xs font-medium">Or pay using UPI ID</span>
                      <div className="flex-1 h-px bg-[#C7D2FE]" />
                    </div>
                    ...
                    <p className="text-[#765334] text-xs text-center -mt-2">
                ...
                <div className="bg-[#FAF3E4] border border-[rgba(8,43,73,0.15)] rounded-2xl p-5 shadow-sm space-y-4">
                  ...
                  <label className="flex flex-col items-center justify-center gap-2 p-6 border-2 border-dashed border-[rgba(8,43,73,0.15)] hover:border-[#C5963A]/50 rounded-xl cursor-pointer transition-all bg-[#F4E9D2]">
                    ...
                    {screenshotPreview ? (
                      ...
                      <>
                        <div className="w-12 h-12 rounded-xl bg-[#FAF3E4] flex items-center justify-center">
                          <Upload size={20} className="text-[#C5963A]" />
                        </div>
                        <p className="text-[#765334] text-sm font-medium">Click to upload payment screenshot</p>
                        <p className="text-[#765334] text-xs">PNG, JPG - max 10MB</p>
                      </>
                    )}
                  </label>
                </div>
        ...
        <div className="bg-[#FAF3E4] border border-[rgba(8,43,73,0.15)] rounded-xl p-6 h-fit sticky top-20 shadow-sm">
            ...
            {items.map(item => {
              ...
              return (
                <div key={item.id || item.product_id} className="flex justify-between text-sm">
                  <span className="text-[#765334] truncate mr-2">{item.products?.name} &times; {item.quantity}</span>
                  <div className="text-right flex-shrink-0">
                    {itemDiscount > 0 && <p className="text-gray-400 line-through text-xs">{formatINR(itemOriginal)}</p>}
                    <span className={`font-medium ${itemDiscount > 0 ? 'text-green-600' : 'text-[#292725]'}`}>{formatINR(itemFinal)}</span>
                  </div>
                </div>
              )
            })}
              ...
              return (
                <>
                  <div className="flex justify-between text-sm">
                    <span className="text-[#765334]">Subtotal</span>
                    <span className="text-[#292725] font-medium">{formatINR(total)}</span>
                  </div>
                  {discount > 0 && (
                  ...
                  <div className="flex justify-between text-sm">
                    <span className="text-[#765334]">Delivery</span>
                    {shipping === 0
                      ? <span className="text-green-600 font-medium">Free</span>
                      : <span className="text-[#C5963A] font-medium">+{formatINR(shipping)}</span>
                    }
                  </div>
          ...
          <div className="mb-4 space-y-3">
            ...
            {allPromoCodes.length > 0 && (
              <div>
                <p className="text-xs text-[#765334] font-semibold mb-2 flex items-center gap-1">
                  <Ticket size={12} className="text-[#C5963A]" /> Available Offers
                </p>
                <div className="space-y-2">
                    ...
                    return (
                      <div key={code.id}
                        className={`border rounded-xl px-3 py-2.5 transition-all ${
                          eligible
                            ? "bg-[#FFF8F0] border-[#C5963A]/40"
                            : "bg-gray-50 border-gray-200 opacity-60"
                        }`}>
                        <div className="flex items-center justify-between gap-2">
                          <div className="flex items-center gap-2 flex-1 min-w-0">
                            <span className={`font-mono font-bold text-xs px-2 py-0.5 rounded flex-shrink-0 ${eligible ? "text-[#C5963A] bg-[#C5963A]/10" : "text-gray-400 bg-gray-200"}`}>
                              {code.code}
                            </span>
                            <div className="min-w-0">
                              <p className={`text-xs font-medium truncate ${eligible ? "text-[#292725]" : "text-gray-400"}`}>
  ...
  />
          ...
          <p className="text-[#765334] text-xs text-center">-??? UPI Payment - Secure &amp; Safe</p>
        </div>
      </div>
    </div>
  )
